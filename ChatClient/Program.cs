...
");
            string user = args.Length > 0 ? args[0] : "Dev2";
            ...
            using (var system = ActorSystem.Create("MyClient", config))
            {
                ...
                try
                {
                    chatClient.Tell(connectRequest);
                }
                catch (Exception)
                {
    ...
    class ChatClientActor : ReceiveActor, ILogReceive
    {
        ...
        private readonly ActorSelection _server = Context.ActorSelection("akka.tcp://MyServer@10.30.12.13:8081/user/ChatServer");
